...
async function startNotificationSystem(role, email) {
    ...
    onSnapshot(q, (snapshot) => {
        ...
        snapshot.docChanges().forEach((change) => {
            ...
            if (role === 'admin' && change.type === "added") {
                ...
                hasNewChange = true;
                let msgTitle = (data.status === "Success" || data.status === "In Progress") ? "ใบงานได้รับการตรวจสอบแล้ว" : "มีการ Update ใบงานแล้ว";
                let theme = (data.status === "Success" || data.status === "In Progress") ? "emerald" : "blue";
                ...
                html += `
                    <div onclick="window.location.href='dashboard.html'" class="p-4 border-b border-slate-50 hover:bg-${theme}-50 transition cursor-pointer">
                        <div class="font-bold text-${theme}-600 text-xs mb-1">🔔 ${msgTitle}</div>
                        <div class="text-slate-700 font-bold text-[11px] mb-1 italic">"${topic}"</div>
                        <div class="text-slate-500 text-[10px] line-clamp-2">Internet No: ${internetNo}</div>
                    </div>`;
            }
        });
    ...
    // 3. ระบบเปิด/ปิด Dropdown และปุ่ม Clear All
    if (notiBtn && notiDrop) {
        notiBtn.onclick = (e) => {
            e.stopPropagation();
        ...
        if (clearBtn) {
            clearBtn.onclick = (e) => {
                e.stopPropagation();
                notiList.innerHTML = `<div class="p-6 text-center text-slate-400 text-xs italic font-medium">ล้างการแจ้งเตือนแล้ว</div>`;
                if (notiDot) notiDot.classList.add('hidden');
            };
        }
